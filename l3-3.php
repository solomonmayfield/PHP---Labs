...
{#form submits here we show entered name
	...
	*/
	...
	//static vars allow data to accrue as totals  
	private static $TotalHomers = 0; //Will Store Total Homers
	private static $TotalPlayers = 0; //Will Store Total BallPlayers
	private $name = '';  //private so user can't change during program
 	private $team = '';
 	private $homers = 0;  //allowing change to homers during runtime would mess up static variable
    function __construct($name,$team,$homers)
	{//constructor sets stage by adding data to an instance of the object
		if($name == ""){die(trigger_error("name property of BallPlayer object does not allow empty string", E_USER_ERROR));}	
	    if($team == ""){die(trigger_error("team property of BallPlayer object does not allow empty string", E_USER_ERROR));}	
		if(!is_int($homers) || $homers < 0){die(trigger_error("homers property of BallPlayer object must be a positive whole number (integer)", E_USER_ERROR));}
	...
	public static function averageHomers()
	{//static function allows us to see the average homers of all players
		return 	number_format(self::$TotalHomers/self::$TotalPlayers,2);	
	}
	
	public function toString()
	{//allows us to see all properties of the object
		return 	" name: " . $this->name . " team: " . $this->team . " homers: " . $this->homers;	
	}
	
	public static function totalHomers()
	{//static function to allow read only access to total homers
		return self::$TotalHomers;
	}

	public static function totalPlayers()
	{//static function to allow read only access to total players
